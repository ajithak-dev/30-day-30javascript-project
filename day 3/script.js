...
function randomColor(){
    ...
    //Math.random return floot value from 0-1
    //*colors.length gives number items in colors the multiply by random value (i.e 0.4*4=4.4)
    //Math.floor round up the floor value from 4.4 to 4
    //Then all happen in color array so that return one index value
    bg.style.setProperty('background-color', color)
   btn.style.setProperty('background-color', color)
